...
@section('content')
    <div class="container-fluid">
        ...
        <x-tab-navigation :tabs="[
            ...
            [
                ...
                'label' => 'Email History',
                'icon' => 'bi-envelope',
                'badge' => $invoice->communications->where('type', 'email')->count(),
            ],
        ]" id="invoiceTabs" />
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6 col-lg-4">
                        <x-editable-info-card label="Due Date" :value="$invoice->due_date ? $invoice->due_date->format('M d, Y') : '-'" field="due_date" entityType="invoices"
                            :entityId="$invoice->id" fieldType="date" :editable="auth()->user()->can('edit invoices')" />
                    </div>
                    {{-- <div class="col-md-6 col-lg-4">
                    <x-info-card
                        label="Status"
                    ...
                    <div class="col-md-6 col-lg-4">
                        <x-editable-info-card label="Status" :value="ucfirst($invoice->status)" field="status" entityType="invoices"
                            :entityId="$invoice->id" fieldType="select" :options="['unpaid' => 'Unpaid', 'sent' => 'Sent', 'paid' => 'Paid', 'draft' => 'Draft']" :badge="ucfirst($invoice->status)" :badgeColor="$invoice->status === 'paid' ? 'success' : ($invoice->status === 'sent' ? 'info' : ($invoice->status === 'unpaid' ? 'warning' : 'secondary'))"
                    ...
                    <div class="col-md-6 col-lg-4">
                        <x-editable-info-card label="Start Date" :value="$invoice->billing_period_start
                            ? $invoice->billing_period_start->format('M d, Y')
                            : '-'" field="billing_period_start"
                            entityType="invoices" :entityId="$invoice->id" fieldType="date" :editable="auth()->user()->can('edit invoices')" />
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <x-editable-info-card label="End Date" :value="$invoice->billing_period_end ? $invoice->billing_period_end->format('M d, Y') : '-'" field="billing_period_end"
                            entityType="invoices" :entityId="$invoice->id" fieldType="date" :editable="auth()->user()->can('edit invoices')" />
                    </div>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr>
                                        <td>
                                            @if (auth()->user()->can('edit invoices'))
                                                <x-editable-info-card label="" :value="'Cleaning Services'" field="description"
                                                    entityType="invoices" :entityId="$invoice->id" fieldType="text"
                                                    :editable="true" />
                                            @else
                                                Cleaning Services
                                            @endif
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end" colspan="2">
                                            @if (auth()->user()->can('edit invoices'))
                                                <x-editable-info-card label="" :value="number_format($invoice->tax ?? 0, 2)" field="tax"
                                                    entityType="invoices" :entityId="$invoice->id" fieldType="number"
                                                    :editable="true" />
                                            @else
                    ...
                    <div class="col-12">
                        <x-editable-info-card label="" :value="$invoice->notes ?? '-'" field="notes" entityType="invoices"
                            :entityId="$invoice->id" fieldType="textarea" :editable="auth()->user()->can('edit invoices')" />
                    </div>
                </div>
            </div>
                    ...
                    <div class="text-center py-5">
                        ...
                        @if (auth()->user()->can('edit invoices'))
                            <button class="btn btn-primary mt-2" data-bs-toggle="modal"
                                data-bs-target="#sendInvoiceEmailModal">
    ...
    @if (auth()->user()->can('edit invoices'))
        <div class="modal fade" id="sendInvoiceEmailModal" tabindex="-1" aria-labelledby="sendInvoiceEmailModalLabel"
            aria-hidden="true">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#sendInvoiceEmailForm').on('submit', function(e) {
                ...
                axios.post('{{ \App\Helpers\RouteHelper::url('invoices.send-email', $invoice) }}',
                        formData, {
                            headers: {
                        ...
                        if (response.data.success) {
                            ...
                            if (typeof window.showToast !== 'undefined') {
                                window.showToast('success',
                                    'Invoice sent successfully! Status updated to Sent.');
                            }
