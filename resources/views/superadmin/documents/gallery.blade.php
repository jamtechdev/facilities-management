                ...
                <div class="card border-0 shadow-sm sticky-top" style="top: 20px;">
                    ...
                    <div class="card-body">
                        ...
                        @if (request('type') || request('entity_type') || request('search'))
                            <div class="mt-4">
                                <a href="{{ \App\Helpers\RouteHelper::url('documents.gallery') }}"
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 document-gallery-header">
                    <div>
                        <p class="text-muted mb-0">
                            ...
                            Showing <strong>{{ $documents->firstItem() ?? 0 }}-{{ $documents->lastItem() ?? 0 }}</strong>
                                    ...
                                    <div class="document-preview">
                                        @if (str_starts_with($document->file_type, 'image/'))
                                            <img src="{{ asset('storage/' . $document->file_path) }}"
                                                alt="{{ $document->name }}" class="document-image"
                                        ...
                                        <div class="document-overlay">
                                            <div class="document-actions">
                                                <a href="{{ \App\Helpers\RouteHelper::url('documents.download', $document) }}"
                                                    class="btn btn-sm btn-light" target="_blank" title="Download">
                                                    <i class="bi bi-download"></i>
                                                </a>
                                                @if (auth()->user()->can('delete documents'))
                                                    <button type="button" class="btn btn-sm btn-light btn-delete-document"
                                                        data-document-id="{{ $document->id }}"
                                                        data-document-name="{{ $document->name }}" title="Delete">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="document-meta">
                                            <span class="badge d-inline-block">
                                                <i
                                                    class="bi bi-tag-fill me-1"></i>{{ ucfirst(str_replace('_', ' ', $document->document_type)) }}
                                            </span>
        ...
        document.querySelectorAll('.btn-delete-document').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (confirm('Are you sure you want to delete "' + documentName +
                    ...
                    form.method = 'POST';
                    form.action = '{{ \App\Helpers\RouteHelper::url('documents.destroy', ':id') }}'
